...
// TestStartPProfServer_Disabled 测试禁用pprof
func TestStartPProfServer_Disabled(t *testing.T) {
	cfg := gopprof.Default().Disable()

	err := StartPProfServer(cfg)
	assert.NoError(t, err, "禁用时应返回nil")
}

// TestStartPProfServer_NoPort 测试未配置端口
func TestStartPProfServer_NoPort(t *testing.T) {
	cfg := gopprof.Default().Enable()
	cfg.Port = 0

	err := StartPProfServer(cfg)
	assert.NoError(t, err, "未配置端口时应返回nil")
}
...
func TestStartPProfServer_WithIPWhitelist(t *testing.T) {
	cfg := gopprof.Default().
		...
		WithPort(16062).
		WithPathPrefix("/debug/pprof").
		WithAllowedIPs([]string{"127.0.0.1", "192.168.1.0/24"})
